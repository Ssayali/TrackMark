package in.blazonsoftwares.trackmark;


public class Configvolley {


    public static final String Product_No = "Product_No";
    public static final String Product_Name = "Product_Name";
    public static final String Product_Spec = "Product_Spec";
    public static final String Product_Image = "Product_Image";
    public static final String Product_price = "Product_price";
    public static final String Product_Shopcode = "Product_Shopcode";
    public static final String Product_MasterId = "Product_MasterId";
    public static final String Product_qty = "Product_qty";
    public static final String Product_addPrice = "Product_Price";

    //shop pojo class
    public static final String Shop_Name = "Shop_Name";
    public static final String Shop_Add = "Shop_Add";
    public static final String Shop_Cno = "Shop_Cno";
    public static final String Shop_Image = "Shop_Image";


    //shop pojo class
    public static final String User_Code = "User_Code";
    public static final String User_Email = "User_Email";
    public static final String User_Name = "User_Name";
    public static final String User_Address = "User_Address";
    public static final String User_cno = "User_cno";

    //map activity code
    public static final String Latitude = "Latitude";
    public static final String Langitude = "Langitude";
    public static final String Shop_no = "Shop_no";
    public static final String Off_Image = "Off_Image";


    //member pojo class
    public static final String Member_Code = "Member_Code";
    public static final String Member_Email = "Member_Email";
    public static final String Member_password = "Member_password";
    public static final String Member_cno = "Member_cno";
    public static final String Member_Address = "Member_Address";

    //member pojo class
    public static final String Cart_Product_Id = "Cart_Product_Id";
    public static final String Cart_Product_qty = "Cart_Product_qty";
    public static final String Cart_Product_Price = "Cart_Product_Price";
    public static final String Cart_Product_Name = "Cart_Product_Name";
    public static final String Cart_Code = "Cart_Code";



}
